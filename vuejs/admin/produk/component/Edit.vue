			  	...
			  	<div class="form-panel">
	              	...
	              	<form class="form-horizontal" v-on:submit.prevent="simpan">
						...
						<div class="form-group">
							...
							<div class="col-md-10">
								<select class="form-control" id="kategori" v-model="data.kategori">
									<option v-for="item in kategori" :value="item.url">{{ item.nama }}</option>
								</select>
							</div>
						</div>
						<div class="form-group">
	...
	export default{
		...
		methods:{
			...
			simpan(){
				...
				this.$http.post('/'+this.id, formData)
				.then(res => {
					this.$swal({
