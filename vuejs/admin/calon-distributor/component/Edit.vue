	              	...
	              	<form class="form-horizontal" v-on:submit.prevent="simpan">
						...
						<div v-for="(item, idx) in kriteria" class="form-group">
							...
							<div class="col-md-10">
								...
								<select class="form-control" @change="handleChange(idx, $event.target.value, item.id, item.kriteria)">
									<option v-for="(jawaban, index) in JSON.parse(item.jawaban)" :value="index + 1" 
									:selected="(typeof data.kriterias[idx] != 'undefined') ? data.kriterias[idx].nilai == (index + 1): ''">
								{{ jawaban.jawaban }}</option>
								</select>
		...
		methods:{
			...
			getData(){
				...
				.then(res => {
					...
					Vue.set(that.$data, 'data', res.data.distributor)
				})
			},
			getDataProvinsi(){
				let that = this
